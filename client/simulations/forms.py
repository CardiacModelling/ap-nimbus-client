...
class IonCurrentForm(forms.ModelForm, UserKwargModelFormMixin):# max/min val for floats
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['current'].widget.attrs = {'title': '> 0 for IC50.', 'required': 'required',
                                               'class': 'current-concentration'}

        self.fields['hill_coefficient'].widget.attrs = {'min': 0.1, 'max': 5.0, 'step': 0.1, 'required': 'required', 'disabled': 'disabled'}
#,
#                                                        'title': 'Between 0.1 and 5.'}

        self.fields['saturation_level'].widget.attrs = \
            {'min': 0.0, 'step': 1.0, 'required': 'required', 'disabled': 'disabled'}
#,
#             'title': "Level of peak current relative to control at a very large compound concentration (between 0 and"
...
#                      "- For an activator Minimum > 100% (no effect) to Maximum 500% (as a guideline)."}
        ...
        self.fields['spread_of_uncertainty'].widget.attrs = \
            {'min': 0.0000000000000001, 'max': 2.0, 'step': 0.01, 'required': 'required', 'disabled': 'disabled'}
#,
#             'title': "Spread of uncertainty (between 0 and 2).\nDefaults are estimates based on a study by Elkins et "
#                      "all.\nIdeally all these numbers would be replaced based on the spread you observe in fitted "
#                      "pIC50s.", 'class': 'spread_of_uncertainty'}

        for _, field in self.fields.items():
            field.widget.attrs['title'] = field.help_text

    def save(self, simulation=None, **kwargs):
        # don't save diabled current
...
class SimulationForm(forms.ModelForm, UserKwargModelFormMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['maximum_pacing_time'].widget.attrs = {'min': 0.0000000000000001, 'max': 120.0, 'step': 1.0, 'required': 'required'}
        for _, field in self.fields.items():
            field.widget.attrs['title'] = field.help_text
